...
function isValidSemVer(value) {
    ...
    const parsedVersion = semver_default().parse(value, { loose: false });
    return !parsedVersion || parsedVersion.version !== value;
}
/**
...
async function getDiff(tag, packagesDir) {
    ...
    const diff = (await git.diff([tag, HEAD, '--name-only', '--', packagesDir])).split('\n');
    DIFFS.set(tag, diff);
    return diff;
}
async function initGit() {
    if (!git) {
        git = src_default()({ baseDir: WORKSPACE });
        [TAGS] = await getTags();
    }
}
async function getTags() {
    const { all, latest } = await git.tags();
    if (!latest || !isValidSemVer(semver_default().clean(latest))) {
        throw new Error(`Invalid latest tag. Expected a valid SemVer version. Received: ${latest}`);
    }
    return [all, latest];
}
