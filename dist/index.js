...
function isValidSemVer(value) {
    ...
    return ((_a = semver_default().parse(value, { loose: false })) === null || _a === void 0 ? void 0 : _a.version) === value;
}
/**
...
async function performDiff(packageData, tag, packagesDir) {
...
async function getTags() {
    ...
    if (!latestTag || !isValidSemVer(semver_default().clean(latestTag))) {
 ...
 * the version.
 */
async function updatePackages(newVersion, allPackages, packagesToUpdate) {
    await Promise.all(Object.keys(packagesToUpdate).map(async (packageName) => updatePackage(newVersion, allPackages[packageName])));
}
async function updatePackage(newVersion, packageMetadata) {
    await external_fs_.promises.writeFile(external_path_default().join(packageMetadata.path, PACKAGE_JSON), JSON.stringify({ ...packageMetadata.manifest, version: newVersion }, null, 2));
}
/**
...
function validatePackageManifest(path, manifest, requiredFields = ['name', 'version']) {
    ...
    if (requiredFields.includes('version') && !isValidSemVer(manifest.version)) {
        throw new Error(`"${manifest.name || path}" manifest version is not a valid SemVer version: ${manifest.version}`);
    }
}
...
async function main() {
    ...
    if (INPUTS.BUMP_TYPE) {
        ...
        newVersion = semver_default().inc(currentVersion, INPUTS.BUMP_TYPE);
    }
    else {
        newVersion = INPUTS.BUMP_VERSION;
    }
    const allPackages = await getPackagesMetadata();
    const packagesToUpdate = await getPackagesToUpdate(allPackages, INPUTS.BUMP_TYPE);
